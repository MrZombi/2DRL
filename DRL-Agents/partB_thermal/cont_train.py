# partB_thermal/cont_train_thermal.py (FINAL)
...
from env_thermal import ThermalEnv, ThermalConfig
from callbacks import ThermalTB

TOTAL_STEPS_DEFAULT = 200_000
MAX_EP_STEPS = 200
FRAME_RUNS_SUBDIR = "B_thermal"

ALGOS = {
    "sac": SAC,
    "td3": TD3,
    "ppo": PPO,
    "a2c": A2C,     # optional core SB3 (continuous supported)
    "ddpg": DDPG,   # optional core SB3
}
...
def train(algo: str, total_steps: int, resume: bool, run_name: str|None, seed: int,
    ...
    # Common policy net arch to make comparisons fair
    policy_kwargs = dict(net_arch=[128, 128])

    # Env config
    cfg = ThermalConfig(horizon_steps=MAX_EP_STEPS, seed=seed)
    # One-off sanity check
    check_env(ThermalEnv(cfg))

    # Build vectorized envs (+Normalize for obs only, consistent across algos)
    ...
    # Dump config + run meta (JSON) for reproducibility
    meta = dict(
        algo=algo,
        seed=seed,
        n_envs=n_envs,
        total_steps=total_steps,
        policy_kwargs=dict(net_arch=[128, 128]),  # falls du es so gesetzt hast
        hp=hp,  # kann Objekte enthalten -> default=str unten
        cfg=asdict(cfg)  # statt vars(cfg)
    )
    ...
    # Callback
    tb_cb = ThermalTB(log_every_steps=1000)
    ...
    # Save both generic and seed-specific filenames
